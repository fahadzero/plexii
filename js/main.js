(function () {
  ...
  var carouselTrack    = carouselViewport && carouselViewport.querySelector('.news-carousel-track');

  if (carouselViewport && carouselTrack) {
    var baseCards       = Array.from(carouselTrack.querySelectorAll('.news-card'));
    ...
    function buildClones() {
      carouselTrack.querySelectorAll('.news-card--clone').forEach(function (c) { c.remove(); });
    ...
    function startTimer()  { carouselTimer = setInterval(advance, 3500); }
    function clearTimer()  { clearInterval(carouselTimer); carouselTimer = null; }
